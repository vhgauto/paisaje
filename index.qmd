...
Para el cálculo de las métricas se utiliza el paquete [`{landscapemetrics}`](https://r-spatialecology.github.io/landscapemetrics/). Las métricas del paisaje a nivel de clase devuelven un valor asociado a cada categoría del ráster de interés.

Se calcularon: [**Mean of patch area**](https://r-spatialecology.github.io/landscapemetrics/reference/lsm_c_area_mn.html) ($AREA_{MN}$) y [**Number of patches**](https://r-spatialecology.github.io/landscapemetrics/reference/lsm_c_np.html) ($NP$).

$$
AREA_{MN} = mean(AREA[patch_{ij}])
$$

Siendo $AREA[patch_{ij}]$ el área de cada parche, en hectáreas, y $AREA_{MN}$ mayor a cero.

$$
NP = n_{i}
$$

Con $n_{i}$ el número de parches, y $NP$ resulta mayor a 1.
...
read_tsv("indices/df_area_mn.tsv", show_col_types = FALSE) |>
  ...
  cols_label(
    dependencias ~ md("Dependencia<br>del terreno")
  )
...
Las métricas a nivel del paisaje obtenidas por el paquete `{landscapemetrics}` calculan un único valor para cada ráster procesado.

En particular nos interesan dos métricas del paisaje: [**Shannon's diversity index**](https://r-spatialecology.github.io/landscapemetrics/reference/lsm_l_shdi.html) ($SHDI$) y [**Patch richness density**](https://r-spatialecology.github.io/landscapemetrics/reference/lsm_l_prd.html) ($PRD$). Sus expresiones se muestran a continuación.

$$
SHDI = - \sum \limits_{i = 1}^{m} (P_{i} \cdot \ln P_{i})
$$
Siendo  $P_{i}$ la proporción de la clase $i$. El valor de $SHDI$ es superior o igual a cero.


$$
PRD = \frac{m} {A} \cdot 10000 \cdot 100
$$

Con $m$ el número de clases y $A$ el área total en m<sup>2</sup>. El valor de $PRD$ es en unidades por 100 hectáreas.

Para los cuatro escenarios propuestos de dependencia del terreno variable, los resultados son:

```{r}
#| eval: false
#| echo: true

f_shannon <- function(x) {
  lsm_l_shdi(land_raster[[x]]) |> 
    select(shannon = value) |> 
    mutate(dependencia_terreno = dependencias[x])
}

df_shannon <- map(1:length(dependencias), f_shannon) |> 
  list_rbind()

f_richness <- function(x) {
  lsm_l_prd(land_raster[[x]]) |> 
    select(richness = value) |> 
    mutate(dependencia_terreno = dependencias[x])
}

df_richness <- map(1:length(dependencias), f_richness) |> 
  list_rbind()

inner_join(df_shannon, df_richness, by = join_by(dependencia_terreno)) |> 
  write_tsv("indices/df_shannon_richness.tsv")
```

```{r}
read_tsv("indices/df_shannon_richness.tsv", show_col_types = FALSE) |> 
  relocate(2, 1, 3) |> 
  gt() |> 
  fmt_number(
    columns = shannon,
  ...
  cols_label(
    dependencia_terreno ~ md("Dependencia<br>del terreno"),
    shannon = md("Shannon's<br>diversity index"),
    richness = md("Patch richness<br>density")
  )
```
