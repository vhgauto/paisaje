...
m_clase |>
  ...
  select(Métrica = titulo, Forest, Agriculture, Urban) |> 
  gt() |> 
  fmt_auto(
    columns = c(Forest, Agriculture, Urban),
    locale = "es"
  ) |> 
  tab_style(
    locations = cells_body(columns = c(Forest, Agriculture, Urban)),
    style = cell_text(font = "JetBrains Mono", color = "black")
  ) |> 
  tab_style(
...
Los ráster se vectorizan y almacenan para una rápida lectura posterios.
...
Se emplea el paquete [`{exactextractr}`](https://isciences.gitlab.io/exactextractr/), que permite una extracción de valores de píxel **rápida** y precisa.
...
f_gg_nutrientes <- function(nutriente) {
  ...
  labs(x = NULL, y = NULL, title = nutriente) +
  theme_bw() +
  theme(
    aspect.ratio = 1,
...
g <- patchwork::wrap_plots(nutri_gg, ncol = 1) &
  theme(
...
![Nutrientes por clase y escenario.](figuras/nutrientes_escenarios_clase.png)
