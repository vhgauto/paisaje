...
## Métricas de <u>clase</u> y <u>paisaje</u> para uso de suelo
...
read_csv("datos/metrica_clase_lulc.csv", show_col_types = FALSE) |> 
  ...
  tab_source_note(
    source_note = md(
      "[Datos (.csv)](https://raw.githubusercontent.com/vhgauto/paisaje/refs/heads/main/datos/metrica_clase_lulc.csv) disponibles para su descarga"
    )
  ) |> 
  tab_options(
...
read_csv("datos/metrica_paisaje_lulc.csv", show_col_types = FALSE) |> 
  ...
  tab_source_note(
    source_note = md(
      "[Datos (.csv)](https://raw.githubusercontent.com/vhgauto/paisaje/refs/heads/main/datos/metrica_paisaje_lulc.csv) disponibles para su descarga"
    )
  ) |> 
  tab_options(
    table.background.color = "transparent"
...
m_clase |>
  ...
  tab_source_note(
    source_note = md(
      "[Datos (.csv)](https://raw.githubusercontent.com/vhgauto/paisaje/refs/heads/main/datos/métricas_X_cla.csv) disponibles para su descarga"
    )
  ) |> 
  tab_options(
    table.background.color = "transparent"
...
m_paisaje |>
  ...
  tab_source_note(
    source_note = md(
      "[Datos (.csv)](https://raw.githubusercontent.com/vhgauto/paisaje/refs/heads/main/datos/métricas_X_pai.csv) disponibles para su descarga"
    )
  ) |> 
  tab_options(
    table.background.color = "transparent"
...
```{r}
nutri_files <- list.files("datos/", pattern = "nutriente", full.names = TRUE)
...
```{r}
escenarios_v <- c(.1, .3, .7, 1)
escenarios10_v <- c(1, 3, 7, 10)
...
gt(area_gt_tbl, groupname_col = "prod", rowname_col = "tipo") |> 
  ...
  tab_style(
    ...
    locations = purrr::map(
      ...
      \(fila) {
        cells_body(
          columns = which(area_gt_tbl[fila, 3:6] > area_gt_tbl$ref[fila])+2,
          rows = fila
        )
      }
    )
