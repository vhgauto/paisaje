...
f_gg_area <- function(i_nutriente) {
  read_csv(area_clase_csv, show_col_types = FALSE) |>
    mutate(
      esc = case_match(
        escenario,
        1 ~ 1,
    ...
    mutate(
      porcentaje = area / area_tot
    ) |>
    filter(nutriente == i_nutriente) |>
    ggplot(aes(esc, porcentaje)) +
    geom_hline(
      data = area_clase_tbl, aes(yintercept = area_prop),
      color = "grey30", linetype = 2, linewidth = 1
    ) +
...
area_gt_tbl <- read_csv(area_clase_csv, show_col_types = FALSE) |>
  mutate(
    esc = case_match(
      escenario,
      1 ~ 1,
  ...
  pivot_wider(
    ...
    id_cols = c(nutriente, tipo)
  ) |> 
  full_join(area_ref_tbl, by = join_by(tipo)) |> 
  mutate(nutriente = toupper(nutriente)) |> 
  mutate(
    nutriente = factor(
      nutriente, levels = c(
        "N_TOTAL_EXPORT", "NDR_N", "P_SURFACE_EXPORT", "NDR_P"
      )
    )
  )

gt(area_gt_tbl, groupname_col = "nutriente", rowname_col = "tipo") |> 
  tab_stub_indent(indent = 5, rows = everything()) |> 
  fmt_percent(
  ...
  cols_label(
    ref = html("<i>% de la clase</i>")
  ) |> 
  tab_footnote(
    footnote = html(
      "Texto destacado indica que en ese escenario, el nutriente asociado 
      a la clase es <b style='font-family: JetBrains Mono; color:
      #8E063B'>mayor</b> al porcentaje de superficie correspondiente."
    )
  ) |> 
  tab_options(
    table.width = 650,
...
En todos los casos, `Grassland` y `Urban` presentan un porcentaje del total de nutrientes (ligeramente) superior a su superficie ocupada.
