...
Se leyeron los ráster de acuerdo al escenario y el nutriente, extrayendo el área asociada a cada clase. Para ello se creó una función personalizada.

```{r}
# escenarios_v <- seq(.1, 1, .1)
# escenarios10_v <- 1:10
# 
# f_area_lista <- function(id, nutriente) {
#   raster <- paste0(
#     "nutrientes/Escenario ",
#     escenarios_v[id],
#     "/",
...
Para cada escenario y tipo de nutriente se obtuvo el área total asignada a cada clase.

```{r}
# f_area_tbl <- function(id, nutriente) {
#   lista <- f_area_lista(id = id, nutriente = nutriente)
# 
#   l <- purrr::map(
#     1:4,
#     \(x) {
...
#         drop_na() |>
#         reframe(
#           area = sum(coverage_fraction * area)
#         ) |>
#         mutate(value = c(-1:2)[x])
#     }
#   )
# 
#   list_rbind(l) |>
...
Se ejecutaron las funciones personalizadas previas, generando `.csv` por nutriente y almacenando para lecturas posteriores.

```{r}
# f_area_csv <- function(nutriente) {
#   df <- purrr::map(
#     1:4,
#     ~ f_area_tbl(id = .x, nutriente = nutriente)
#   )
# 
#   list_rbind(df) |>
#     write_csv(paste0("datos/area-clase-", nutriente, ".csv"))
# 
#   print(paste0("---Datos ", nutriente, " almacenados---"))
# }
